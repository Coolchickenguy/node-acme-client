...
class HttpClient {
    ...
    async createSignedBody(url, payload = null, { nonce = null, kid = null } = {}) {
        ...
        let headerAlg = 'RS256';
        let signerAlg = { name: 'RSASSA-PKCS1-v1_5', hash: { name: 'SHA-256' } };
        ...
        if (jwk.crv && (jwk.kty === 'EC')) {
            ...
            if (jwk.crv === 'P-384') {
                headerAlg = 'ES384';
                signerAlg = { name: 'RSASSA-PKCS1-v1_5', hash: { name: 'SHA-384' } };
            }
            else if (jwk.crv === 'P-521') {
                headerAlg = 'ES512';
                signerAlg = { name: 'RSASSA-PKCS1-v1_5', hash: { name: 'SHA-512' } };
            }
        }
